...
vec3 particlePos(float t, vec3 dir, vec3 sib, float home, float seed) {
  ...
  float r = toCore > 0.5
    ? mix(home, home * 0.28, pow(t, 1.5))                    // fast fall inward
    : mix(home, min(home + 0.34, 1.03), 1.0 - pow(1.0 - t, 2.2)); // thrown out

  // grains fan slightly off their ray as they travel
  vec3 basedir = normalize(dir + sib * (0.1 * t));
